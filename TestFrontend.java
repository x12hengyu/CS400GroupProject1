...
public class TestFrontend {
	...
	public void runTests() {
		...
		System.out.print("Test retriving movie after selecting Horror and rating (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
		if (this.testFrontendSelectingMovie()) {
			System.out.println("PASSED");
		} else {
			System.out.println("FAILED");
		}
	...
	/**
	 * This is the method to test intializer page of rating.
	 * this method would check that all rating is all shown on the screen
	 * if any of the rating is missing, then this would reply back as false
	 * otherwise this would be true
	 * Note: this method will return nothing if x is not exiting the program correctly
	 */
	...
	/** This method will test out the search function when select horror genre.
	 * This methods will goes to genre selection scene. Select horror (3) Then goes back
	 * Then goes to the rating selection. It will select all movie with horror. Then goes back
	 * This should show all movie we input 
	 */
	public boolean testFrontendSelectingMovie() {
		PrintStream standardOut = System.out;
                InputStream standardIn = System.in;
                try {
                        // set the input stream to our input (with an g to test of the program lists genres)
                        ...
                        // same for standard in
                        System.setIn(standardIn);
			// All this should make all three movie pop up
			String appOutput = outputStreamCaptor.toString();
